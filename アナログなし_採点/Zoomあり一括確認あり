...
def Zoom( FLAG ):
  ...
  if FLAG == 0:
    myPush( Keycode.UP_ARROW )
  else:
    myPush( Keycode.DOWN_ARROW )
...
while True:
  ...
  if FlagAll==True and GPIO[ myKey[1][0] ].fell : Zoom( 0 )
  if FlagAll==True and GPIO[ myKey[1][2] ].fell : Zoom( 1 )
  ...
  #if FlagFlt==True and GPIO[ myKey[1][--] ].fell : MODE[0] = Fltxx( 3 ) #Filter--
