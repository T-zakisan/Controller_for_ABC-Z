...
for ii in range( len(DataList) ) :
  ...
  myKey[1] += [DataList[ii][DataList[ii].find(',')+1:].strip()]
...
while True:
  for ii in range( len( GPIO ) ): #ボタンの数で繰り返し　
    ...
    if GPIO[ii].fell and FlagAll==False and FlagFlt==False: #ボタン「押」 ※フラグはブロック
      ...
      if   myKey[1][0]==str(ii) : myPush( Keycode.UP_ARROW )    #上
      elif myKey[1][1]==str(ii) : myPush( Keycode.LEFT_ARROW )  #左
      elif myKey[1][2]==str(ii) : myPush( Keycode.DOWN_ARROW )  #下
      elif myKey[1][3]==str(ii) : myPush( Keycode.RIGHT_ARROW ) #右
      elif myKey[1][4]==str(ii) : myPush( Keycode.C )  #△
      elif myKey[1][5]==str(ii) : myPush( Keycode.Z )  #〇
      elif myKey[1][6]==str(ii) : myPush( Keycode.X )  #✕
      elif myKey[1][7]==str(ii) : myPush( Keycode.P )  #戻
      # elif myKey[1][8]==str(ii)  : #Filter(L2)
      elif myKey[1][9]==str(ii)  : myPush( Keycode.N )  #次
      # elif myKey[1][10]==str(ii) : #ALL(R2)
